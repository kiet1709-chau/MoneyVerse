...
const Reports = ({ darkMode, setDarkMode, transactions = [] }) => {
  ...
  const [selectedMonth, setSelectedMonth] = useState(
    localDate.toISOString().slice(0, 7),
  );
  ...
  const report = useMemo(() => {
    ...
    const average = expenseTransactions.length
    ...
    const periodGrouped = filtered.reduce((result, item) => {
      ...
      result[label][item.type === "income" ? "income" : "expense"] += Number(
        item.amount || 0,
      );
      return result;
    }, {});
    ...
    const chartData = chartKeys.map((label) => [
      label,
      periodGrouped[label] || { income: 0, expense: 0 },
    ]);
    return {
      filtered,
      total: totalExpense,
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200 font-sans">
      <header className="bg-white/80 dark:bg-gray-800/80 border-b border-gray-200 dark:border-gray-700 p-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="bg-sky-700 w-10 h-10 rounded-full text-white font-bold"
          >
            AD
          </button>
        </div>
      ...
      <main className="p-6 md:p-8 max-w-6xl mx-auto space-y-6 flex flex-col [&>section:nth-child(1)]:order-1 [&>section:nth-child(2)]:hidden [&>section:nth-child(3)]:order-4 [&>section:nth-child(4)]:order-2">
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            {[
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl font-semibold ${period === value ? "bg-sky-700 text-white" : "bg-gray-100 dark:bg-gray-700"}`}
              >
                {label}
              </button>
            ))}
            {period === "day" && (
              <input
                ...
                className="px-3 rounded-xl border dark:bg-gray-900"
              />
            )}
            {period === "month" && (
              <input
            ...
            {period === "year" && (
              <input
                ...
                className="w-28 px-3 rounded-xl border dark:bg-gray-900"
              />
            )}
          </div>
          <div className="grid md:grid-cols-3 gap-4">
            <div className="bg-sky-50 dark:bg-sky-900/20 p-5 rounded-xl">
            ...
            <div className="bg-purple-50 dark:bg-purple-900/20 p-5 rounded-xl">
              ...
              <strong className="text-2xl">{report.filtered.length}</strong>
            </div>
        ...
        <section className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
          <h2 className="text-lg font-bold mb-2">Phân tích</h2>
        ...
        <section className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
          <div className="flex items-center justify-between gap-3 mb-4">
            ...
            <button
              onClick={() => navigate("/all-transactions-report")}
              className="px-4 py-2 rounded-xl font-semibold bg-sky-700 text-white"
            >
              Tất cả
            </button>
          </div>
          <div className="grid md:grid-cols-3 gap-4">
            <div className="bg-emerald-50 dark:bg-emerald-900/20 p-5 rounded-xl">
              <p className="text-sm text-gray-500">Tổng tiền thu</p>
              <strong className="text-2xl text-emerald-700">
                {money(report.categoryIncome)}
              </strong>
            </div>
            <div className="bg-rose-50 dark:bg-rose-900/20 p-5 rounded-xl">
              <p className="text-sm text-gray-500">Tổng tiền chi</p>
              <strong className="text-2xl text-rose-700">
                {money(report.categoryExpense)}
              </strong>
            </div>
            <div className="bg-sky-50 dark:bg-sky-900/20 p-5 rounded-xl">
              <p className="text-sm text-gray-500">Còn lại trong tài khoản</p>
              <strong
                className={`text-2xl ${report.categoryIncome - report.categoryExpense >= 0 ? "text-sky-700" : "text-rose-700"}`}
              >
                {money(report.categoryIncome - report.categoryExpense)}
              </strong>
            </div>
          </div>
        </section>
        <section className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
          ...
          {report.chartData.length ? (
            <div className="flex">
              <div className="w-10 h-72 flex flex-col justify-between text-[10px] text-gray-500 pr-1 text-right">
                <span>Triệu</span>
                {(() => {
                  const axisMax =
                    Math.ceil(
                      Math.max(
                        ...report.chartData.flatMap(([, value]) => [
                          value.income,
                          value.expense,
                        ]),
                        1,
                  ...
                  return Array.from({ length: 6 }, (_, index) =>
                    Math.round((axisMax * (5 - index)) / 5),
                  ).map((tick) => (
                    <span key={tick}>{Math.round(tick / 1000000)}</span>
                  ));
                })()}
              </div>
              <div className="h-72 flex-1 flex items-end gap-2 overflow-x-auto border-l border-b border-gray-300 dark:border-gray-600 px-2">
                {report.chartData.map(([label, values]) => {
                  ...
                  const incomeHeight = Math.max(
                    values.income ? 8 : 0,
                    (values.income / max) * 220,
                  );
                  const expenseHeight = Math.max(
                    values.expense ? 8 : 0,
                    (values.expense / max) * 220,
                  );
                  return (
                    <div
                      key={label}
                      onClick={() =>
                        setSelectedChartLabel(
                          selectedChartLabel === label ? null : label,
                        )
                      }
                      className="min-w-[70px] flex-1 h-full flex flex-col justify-end items-center gap-2 cursor-pointer"
                      title={`${label}: Thu ${money(values.income)} - Chi ${money(values.expense)}`}
                    >
                      <div className="flex items-end gap-1 h-[220px]">
                        <div
                          className="w-7 rounded-t-lg bg-emerald-600 hover:bg-emerald-500 transition-all"
                          style={{ height: `${incomeHeight}px` }}
                        />
                        <div
                          className="w-7 rounded-t-lg bg-rose-600 hover:bg-rose-500 transition-all"
