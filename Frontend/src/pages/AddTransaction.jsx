...
const AddTransaction = ({ darkMode, setDarkMode, balance, setBalance, transactions = [], setTransactions }) => {
  ...
  const getBudgetWarning = (transactionCategory, numericAmount) => {
    if (type !== 'expense') return '';
    let budgets = {};
    try { budgets = JSON.parse(localStorage.getItem('moneyverse_category_budgets') || '{}'); } catch { budgets = {}; }
    const budget = Number(budgets[transactionCategory] || 0);
    const currentMonth = new Date().toISOString().slice(0, 7);
    const alreadySpent = transactions
      .filter((item) => item.type === 'expense' && item.category === transactionCategory && String(item.date || '').slice(0, 7) === currentMonth)
      .reduce((total, item) => total + Number(item.amount || 0), 0);
    if (!budget) return `Khoản chi này chưa có ngân sách thiết lập cho hạng mục “${transactionCategory}”.`;
    if (alreadySpent + numericAmount > budget) return `Khoản chi này sẽ vượt ngân sách ${new Intl.NumberFormat('vi-VN').format(alreadySpent + numericAmount - budget)} đ cho hạng mục “${transactionCategory}”.`;
    return '';
  };
  ...
  const handleSubmit = (event) => {
    ...
    const warning = getBudgetWarning(transactionCategory, numericAmount);
      ...
      <main className="max-w-2xl mx-auto bg-white dark:bg-gray-800 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700 p-6 sm:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <select id="transaction-category" value={category} onChange={(event) => { setCategory(event.target.value); setError(''); setBudgetWarning(''); }} className={inputClass}>
              {type === 'income' ? (
                <>
            ...
            {type === 'expense' && category === 'other' && (
              <input
                ...
                onChange={(event) => { setCustomCategory(event.target.value); setError(''); setBudgetWarning(''); }}
          ...
          <div>
            ...
            <input id="transaction-amount" type="number" required min="1" max="999999999" step="1" value={amount} onChange={(event) => { setAmount(event.target.value); setError(''); setBudgetWarning(''); }} placeholder="Ví dụ: 5000000" className={inputClass} />
          </div>
          ...
          {budgetWarning && <div className="rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-900/20 dark:text-amber-100"><p className="font-semibold">⚠ {budgetWarning}</p><p className="mt-1">Bạn vẫn có thể ghi nhận giao dịch này.</p><div className="mt-3 flex gap-3"><button type="button" onClick={() => setBudgetWarning('')} className="rounded-lg px-3 py-2 font-semibold hover:bg-amber-100 dark:hover:bg-amber-900/40">Chỉnh sửa</button><button type="submit" className="rounded-lg bg-amber-600 px-3 py-2 font-semibold text-white hover:bg-amber-700">Vẫn ghi nhận</button></div></div>}
